...
export async function GET(request: Request) {
  ...
  const startDate = searchParams.get("filters[created_at_from]") || "";
  const endDate = searchParams.get("filters[created_at_to]") || "";
  ...
  if (filtersParam && /^\d{4}-\d{2}-\d{2}$/.test(filtersParam)) {
    ...
    filteredUsers = filteredUsers.filter(
      (user) => user.created_at === filtersParam
    );
  }
  ...
  if (endDate && /^\d{4}-\d{2}-\d{2}$/.test(endDate)) {
    filteredUsers = filteredUsers.filter((user) => user.created_at <= endDate);
  }
